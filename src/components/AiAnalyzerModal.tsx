...
export const AiAnalyzerModal: React.FC<AiAnalyzerModalProps> = ({ isOpen, onClose }) => {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
    <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-2xl shadow-2xl p-6 space-y-6">
      ...
      {stats && <p className="text-xs text-slate-500">Last verified: {new Date(stats.checkedAt).toLocaleString()}</p>}
      {stats && <div className="max-h-56 overflow-y-auto rounded-xl border border-slate-800 bg-slate-950/60 p-3 space-y-2"><h3 className="text-sm font-semibold text-white">Active AI Infrastructure</h3>{stats.activeApiKeyList.map((name) => <div key={name} className="flex items-center gap-2 text-xs text-slate-300"><span className="text-emerald-400">●</span><span>{name}</span><span className="ml-auto text-emerald-400">Operational</span></div>)}{stats.activeApiKeyList.length === 0 && <p className="text-xs text-slate-500">Please add at least one API key in the API Portal to enable AI features.</p>}</div>}
      {stats && <p className="text-sm font-semibold text-emerald-400">Verified {stats.activeApiKeyList.length} API Key Providers operational</p>}
