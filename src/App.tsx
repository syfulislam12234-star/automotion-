        ...
        <section className="bg-slate-900/90 border border-slate-800 rounded-2xl p-4 sm:p-5 shadow-lg relative overflow-hidden">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <img src="/logo.png" alt="Naxora AI logo" className="w-14 h-14 shrink-0 object-contain drop-shadow-[0_0_18px_rgba(34,211,238,0.4)]" />
              <div>
        ...
        {(currentUser?.role === 'admin' || currentUser?.isAdmin === true) && activeTab === 'admin' && (
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2 p-1.5 rounded-2xl bg-slate-900/80 border border-slate-800 w-fit">
              ...
              <button
                onClick={() => setAdminSection('adsai')}
                className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${adminSection === 'adsai' ? 'bg-purple-600 text-white shadow' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/70'}`}
              >
                📢 Ads &amp; AI Control
              </button>
              <button
